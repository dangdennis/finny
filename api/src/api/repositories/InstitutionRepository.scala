...
object InstitutionRepository:
  def createInstitution(institution: PlaidInstitution)(using
      session: DBSession
  ): Either[AppError, Institution] =
    val result = Try(
      sql"INSERT INTO institutions (institution_id, name, oauth, logo, primary_color, url, country_codes, dtc_numbers, products, routing_numbers) VALUES (${institution.institutionId}, ${institution.name}, ${institution.oauth}, ${institution.logo}, ${institution.primaryColor}, ${institution.url}, ${institution.countryCodes}, ${institution.dtcNumbers}, ${institution.products}, ${institution.routingNumbers})".update
        .apply()
    ...
    for
      ...
      institution <- getInstitutionById(institutionId)
        .flatMap(
          _.toRight(
            AppError.NotFoundError("Institution not found after creation")
          )
        )
    yield institution
  ...
  def getInstitutionById(
      ...
      session: DBSession
  ): Either[AppError.DatabaseError, Option[Institution]] = Try(
