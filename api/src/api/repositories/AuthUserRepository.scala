...
object AuthUserRepository:
    ...
    def getIdentities(userId: UserId): Either[AppError.DatabaseError, List[AuthIdentity]] = Try(
        DB readOnly { implicit session =>
    ...
    def deleteIdentitiesAdmin(userId: UserId): Either[AppError.DatabaseError, Int] = Try(
