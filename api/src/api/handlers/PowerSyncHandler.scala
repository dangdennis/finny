...
object PowerSyncHandler:
  def handleEventUpload(input: String, user: Profile): Either[HttpError, Unit] =
    ...
    decode[EventUploadRequest](input).flatMap(
      processEventUploadRequest(_, user)
    ) match
      case Left(error) =>
  ...
  private def processEventUploadRequest(
      request: EventUploadRequest,
      user: Profile
  ): Either[AppError, Unit] = request.data
  ...
  private def processEvent(
    ...
    (event.`type`, event.op) match
      case ("goals", "PUT") =>
        event.data
          .toRight(
            AppError.ValidationError("No data found for Goal PUT operation")
          )
          .flatMap(
            _.as[GoalPutData].left.map(err =>
              AppError.ValidationError(err.getMessage)
            )
          )
      ...
      case ("goals", "PATCH") =>
        event.data
          .toRight(
            AppError.ValidationError("No data found for Goal PATCH operation")
          )
          .flatMap(
            _.as[GoalPatchData].left.map(err =>
              AppError.ValidationError(err.getMessage)
            )
          )
        ...
        event.data
          .toRight(
            AppError.ValidationError(
              "No data found for GoalAccount PUT operation"
            )
          )
          .flatMap(
            _.as[GoalAccountPutData].left.map(err =>
              AppError.ValidationError(err.getMessage)
            )
          )
      ...
      case ("goal_accounts", "PATCH") =>
        event.data
          ...
          .flatMap(
            _.as[GoalAccountPatchData].left.map(err =>
              AppError.ValidationError(err.getMessage)
            )
          )
      ...
      case ("profiles", "PATCH") =>
        event.data
          .toRight(
            AppError.ValidationError(
              "No data found for Profile PATCH operation"
            )
          )
          .flatMap(
            _.as[ProfilePatchData].left.map(err =>
              AppError.ValidationError(err.getMessage)
            )
          )
      ...
      case _ =>
        Logger.root.info(
          s"Skipping event ${event.op} ${event.`type`} ${event.id}"
        )
        Right(())

  private def handleGoalPut(
      recordId: String,
      data: GoalPutData,
      user: Profile
  ): Either[AppError, Unit] =
  ...
  private def handleGoalAccountDelete(
      recordId: String,
      user: Profile
  ): Either[AppError, Unit] =
    Logger.root.info(s"Deleting goal account $recordId")
    GoalRepository
      .deleteGoalAccount(UUID.fromString(recordId), user.id)
      .map(_ => ())

  private def handleProfilePatch(
      recordId: String,
      data: ProfilePatchData,
      user: Profile
  ): Either[AppError, Unit] =
    ProfileRepository
      .updateProfile(
        ProfileRepository.ProfileUpdate(
          ...
          dateOfBirth = data.date_of_birth
            .map(epoch =>
              LocalDate
                .ofInstant(Instant.ofEpochSecond(epoch.toLong), ZoneOffset.UTC)
            ),
  ...
  case class EventUpload(
      op_id: Int,
      op: String,
      `type`: String,
      id: String,
      tx_id: Int,
      data: Option[io.circe.Json]
  )
  object EventUpload:
    given Decoder[EventUpload] = deriveDecoder

  case class GoalPutData(
      amount: Double,
      name: String,
      target_date: String,
      goal_type: GoalType
