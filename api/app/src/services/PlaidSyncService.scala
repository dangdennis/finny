...
object PlaidSyncService:
    ...
    def runPlaidSyncPeriodically(): Unit =
        while true do
            ...
            Logger.root.info(
                s"Executing Plaid sync at ${currentTime}"
            )
            PlaidItemRepository
                .getItemsPendingSync(
                    now = currentTime
    ...
    private def _sync(
        ...
        val item =
            PlaidItemRepository.getById(
                itemId
            )
        item match
            case Left(exception) =>
                Logger.root.error(
                    f"Error getting item",
                    exception
                )
                PlaidItemRepository
                    .updateSyncError(
    ...
    private def handleItemResponse(
        ...
        val accounts =
            response.getAccounts.asScala
        var encounteredError =
            (false, "")
        Logger.root.info(
            s"Got number of accounts: ${accounts.size}"
        )
            ...
            AccountRepository.upsertAccount(
                AccountRepository
                    .UpsertAccountInput(
                        ...
                        currentBalance = account.getBalances
                            .getCurrent(),
                        availableBalance = account.getBalances
                            .getAvailable(),
                        isoCurrencyCode = Option(
                            account.getBalances.getIsoCurrencyCode
                        ),
                        unofficialCurrencyCode = Option(
                            account.getBalances.getUnofficialCurrencyCode
                        ),
                        accountType = Option(
                            account.getType.getValue
                        ),
                        accountSubtype = Option(
                ...
                case Failure(error) =>
                    val msg =
                        s"Error upserting account: $error"
                    Logger.root.error(
                        msg
                    )
                ...
                case Success(
                    ...
                    Logger.root.info(
                        s"Upserted account: $accountId"
                    )

        val added_or_modified =
            response.getAdded.asScala ++ response.getModified.asScala
        Logger.root.info(
            s"added_or_modified length: ${added_or_modified.size}"
        )
        val removed =
            response.getRemoved.asScala
        Logger.root.info(
            s"removed length: ${removed.size}"
        )
        ...
        do
            Logger.root.info(
                s"Upserting transaction: $transaction"
            )
            AccountRepository
                .getByPlaidAccountId(
                    itemId = item.id,
                        ...
                        .upsertTransaction(
                            TransactionRepository
                                .UpsertTransactionInput(
                                    ...
                                    unofficialCurrencyCode = Option(
                                        transaction.getUnofficialCurrencyCode
                                    ),
