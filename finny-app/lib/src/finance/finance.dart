...
class Finance {
  ...
  static num gDivGp(num r, num n, num p, num x, num y, num w) {
  ...
  static num rate({
    ...
    while ((iterator < maxIter) && !close) {
      final num rnp1 = rn - gDivGp(rn, nper, pmt, pv, fv, when);
  ...
  static num npvDivNpvPrime(num rate, List<num> values) {
  ...
  static num irr({
    ...
    bool close = false;
    while ((iterator < maxIter) && !close) {
      final num rnp1 = rn - npvDivNpvPrime(rn, values);
      final num diff = (rnp1 - rn).abs();
      close = diff < tol;
      iterator += 1;
      rn = rnp1;
