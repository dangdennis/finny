...
extension PlaidApiEvent {
    struct Migration: AsyncMigration {
        func prepare(on database: Database) async throws {
            try await database.schema(PlaidApiEvent.schema).id().field(
                "item_id",
                .uuid,
                .required,
                ...
                .string,
                .required
            ).field("arguments", .string).field("request_id", .string).unique(on: "request_id")
                .field("error_type", .string).field("error_code", .string).field(
                    "created_at",
                    .datetime,
                    .required,
