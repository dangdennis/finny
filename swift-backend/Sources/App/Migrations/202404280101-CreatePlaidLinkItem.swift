...
extension PlaidLinkEvent {
    struct Migration: AsyncMigration {
        func prepare(on database: Database) async throws {
            try await database.schema(PlaidLinkEvent.schema).id().field(
                "type",
                .string,
                .required
            ).field("user_id", .uuid, .required, .references("users", "id"))
                .field("link_session_id", .string).field("request_id", .string)
                .unique(on: "request_id").field("error_type", .string).field(
                    "error_code",
                    .string
