    ...
    struct Migration: AsyncMigration {
        func prepare(on database: Database) async throws {
            try await database.schema(PlaidItem.schema).id().field(
                ...
                .required,
                .references("users", "id")
            ).field("plaid_access_token", .string, .required).unique(on: "plaid_access_token")
                .field("plaid_item_id", .string, .required).unique(on: "plaid_item_id").field(
                    "plaid_institution_id",
                    .string,
                    .required
                ).field("status", .string, .required).field("transactions_cursor", .string).field(
                    "created_at",
                    .datetime,
                    .required,
