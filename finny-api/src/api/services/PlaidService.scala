...
object PlaidService:
  ...
  def getAccounts(
      client: PlaidApi,
      item: PlaidItem
  ): Either[AppError, AccountsGetResponse] =
  ...
  def getTransactionsSync(
      client: PlaidApi,
      item: PlaidItem
  ): Either[AppError, TransactionsSyncResponse] =
          ...
          .map(error =>
            PlaidApiEventRepository.create(
              PlaidApiEventCreateInput(
                ...
                arguments =
                  Map("cursor" -> item.transactionsCursor.getOrElse("")),
                requestId = Some(error.requestId),
                errorType = Some(error.errorType),
                errorCode = Some(error.errorCode)
              )
  ...
  def getItem(
      ...
      accessToken: String,
      userId: UUID
  ): Either[AppError, ItemGetResponse] =
  ...
  def deleteItem(
      ...
      itemId: PlaidItemId,
      userId: UserId
  ): Either[AppError, Unit] = PlaidItemRepository
  ...
  def getInvestmentHoldings(
      client: PlaidApi,
      item: PlaidItem
  ): Either[AppError, InvestmentsHoldingsGetResponse] =
    PlaidItemRepository
      .getById(item.id.toUUID)
      .flatMap(plaidItem =>
        val req = InvestmentsHoldingsGetRequest()
          .accessToken(plaidItem.plaidAccessToken)
        handlePlaidResponse(
  ...
  ): Either[AppError, InstitutionsGetByIdResponse] =
    val client = makePlaidClientFromEnv()
    val request = new InstitutionsGetByIdRequest()
      .institutionId(institutionId)
      .countryCodes(List(CountryCode.US).asJava)

    handlePlaidResponse(
      Try(client.institutionsGetById(request).execute()),
      respBody =>
        respBody.left
          .map(error =>
            PlaidApiEventRepository.create(
              PlaidApiEventCreateInput(
                userId = None,
                itemId = None,
                plaidMethod = "institutionsGetById",
                arguments = Map("institutionId" -> institutionId),
                requestId = Some(error.requestId),
                errorType = Some(error.errorType),
                errorCode = Some(error.errorCode)
              )
  ...
  object PlaidError:
    ...
    def fromJson(json: String): Either[io.circe.Error, PlaidError] =
      io.circe.parser.decode[PlaidError](json)
